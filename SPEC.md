...
- The worker SHOULD support targeted manual sync requests for a specific mapped Paperclip project, imported Paperclip issue, or Paperclip issue linked directly to a GitHub issue or pull request.
...
- The plugin MUST expose agent tools for the GitHub issue and pull request workflow around synced work, including repository-item search, issue reads and updates, `assign_to_current_user` for assigning issues to the GitHub user authenticated by the saved token, issue comment reads and writes, pull request creation and updates, pull request file and CI inspection, review-thread reads and replies, review-thread resolution changes, `request_pull_request_reviewers` reviewer requests, organization-level GitHub Project search/listing, associating pull requests with organization-level GitHub Projects, and `link_github_item` for linking a Paperclip issue to a GitHub issue or pull request in mapped or third-party repositories.
...
- The plugin MUST expose an agent-authenticated native plugin JSON API route at `/issue-link` for linking a Paperclip issue to a GitHub issue or pull request when authenticated agents cannot call plugin tools. That route MUST use the host-provided authenticated agent company, MUST reject any payload `companyId` that disagrees with that company, MUST verify the live GitHub item, and MUST persist the same durable issue-link or pull-request-link entity that `link_github_item` and hosted manual link actions write.
- The sync flow MUST fetch open GitHub issues from every configured repository.
- The sync flow MUST also include company-scoped GitHub issue and pull request link entities whose repositories are not covered by a configured mapping, and for those third-party repositories it MUST fetch only the linked GitHub issue or pull request instead of importing or scanning the entire repository.
...
- Repeated sync runs MUST continue reconciling Paperclip issues linked directly to third-party GitHub issues from the live GitHub issue state, labels, description, linked pull requests, and trusted comment rules without requiring that GitHub repository to be mapped to a Paperclip project.
...
- An open GitHub issue with a linked PR that still has unfinished CI jobs MUST map to Paperclip `in_progress`, except when the Paperclip issue is already `blocked` and the only adverse PR signal is pending external merge requirements such as GitHub `BLOCKED` or `UNSTABLE`; in that case sync MUST preserve `blocked` instead of waking an executor.
...
- A manual GitHub issue link for a mapped repository MUST write the same issue-link entity and import-registry entry used by normal GitHub issue imports. A manual GitHub pull request link MUST write the same pull-request link entity used by project Pull Requests page create/link flows.
- Agent tool and `/issue-link` route links MAY target third-party repositories that are not mapped to the Paperclip issue's project. Those links MUST still write company-scoped durable issue-link or pull-request-link entities, and sync MUST use those entities as targeted worklist entries rather than requiring a repository mapping.
